...
class paper_scn(Screen):
	...
	def submit(self):
		...
		try:
			...
			popup=Popup(title="Oops!!",content=(Label(text="Check Your Internet Connection")),size_hint=(None,None,),size=(win_size[0]*0.7,win_size[1]*0.2))
			popup.open()
			return
...
class main(App):
	def build(self):
		...
		try:
			...
			label=Label(text="Check Your Internet Connection",text_size=(win_size[0],None),size_hint_x=None,width=win_size[0],color=(0,0,0,1),halign="center")
			home_scn.add_widget(label)
			return sm
			
			# fetching the details of exams and papers
		for i in data:
				if data.index(i)%2==0:
					i=i.rstrip(".kv")
					spt=i.split(" ")
		...
		home_header_box =BoxLayout(orientation="vertical",size_hint_y=None,height=win_size[1]*0.1)
		...
		home_box =BoxLayout(orientation="vertical",padding=(win_size[0]*0.1,win_size[1]*0.35),spacing=win_size[1]*0.005,size_hint_y=None,height=win_size[1]*0.9)
		...
		for i in avai_exam:
			btn=Button(text=i,size_hint_y=None,height=win_size[1]*0.1)
	...
	def instr_scn_build(self,btn):
		...
		if selected_exam==None:
			popup=Popup(title="Oops!!",content=(Label(text="You have not selected any Exam")),size_hint=(None,None,),size=(win_size[0]*0.7,win_size[1]*0.2))
			popup.open()
			return 
		
		if selected_paper==None:
			popup=Popup(title="Oops!!",content=(Label(text="You have not selected any Paper")),size_hint=(None,None,),size=(win_size[0]*0.7,win_size[1]*0.2))
			popup.open()
			return 
		...
		instr_header_box= BoxLayout(orientation="vertical",size_hint_y=None,height=win_size[1]*0.1)
	...
	def other_instr_build(self,btn):
		...
		other_instr_header_box= BoxLayout(orientation="vertical",size_hint_y=None,height=win_size[1]*0.1)	
		...
		other_instr_box= BoxLayout(orientation="vertical",size_hint_y=None,height=win_size[1]*0.4)
		scroll.add_widget(other_instr_box)
		
		box= BoxLayout(orientation="horizontal",size_hint_y=None,height=win_size[1]*0.05)
		other_instr_box.add_widget(box)
		
		label=Label(text="Choose your language: ",size_hint_x=None,width=win_size[0]*0.6,text_size=(win_size[0]*0.6,None),color=(0,0,0,1))
		box.add_widget(label)
		global selected_language
		selected_language=None
		language_drop=DropDown()
		for i in avai_language[selected_exam+selected_paper]:
			btn=Button(text=i,size_hint=(None,None),width=win_size[0]*0.2)
		...
		main_button =Button(text="Language",on_release=language_drop.open,size_hint_x=None,width=win_size[0]*0.2)
		...
		undertaking_check= CheckBox(size_hint_x=None,width=win_size[0]*0.1,color=(0,0,0,1))
		box2.add_widget(undertaking_check)
		
		label=Label(text="I have read and understood the instruction.All computer hardwates alloted to me are in proper working condition.I declare that I am not in possession of /not wearing/not carrying any prohibited gagets like mobile phone,bluetooth devices,etc/any prohibited material with me in the examination hall.I agree in case of not adhering to the instruction, I shall be liable to be debarred from this test and/or to disciplinary action,which may include ban from future tests/examinations",color=(0,0,0,1),text_size=(win_size[0]*0.8,None),size_hint_x=None,width=win_size[0]*0.8)
		box2.add_widget(label)
		
			
		other_instr_box2 =BoxLayout(orientation="horizontal",size_hint_y=None,height=win_size[1]*0.1,spacing=win_size[1]*0.001)
		other_instr_main_box.add_widget(other_instr_box2)
		
		prev_btn=Button(text="Previous",color=(1,1,1),size_hint=(None,None),width=win_size[0]*0.5,height=win_size[1]*0.05,on_release=self.back_to_instr)
		
		other_instr_box2.add_widget(prev_btn)
	
		next_btn=Button(text="Next",background_color=(0,0,0),color=(1,1,1),size_hint=(None,None),width=win_size[0]*0.5,height=win_size[1]*0.05,on_release=self.paper_scn_build)
	...
	def paper_scn_build(self,btn):
		...
		if selected_language==None:
			popup=Popup(title="Oops!!",content=(Label(text="You have not select any language")),size_hint=(None,None,),size=(win_size[0]*0.7,win_size[1]*0.2))
			popup.open()
			return
